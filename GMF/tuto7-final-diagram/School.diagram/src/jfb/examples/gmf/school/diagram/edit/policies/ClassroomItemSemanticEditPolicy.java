 ...
 * @generated
 */
public class ClassroomItemSemanticEditPolicy extends
		SchoolBaseItemSemanticEditPolicy {

	/**
	 * @generated
	 */
	public ClassroomItemSemanticEditPolicy() {
	...
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (SchoolVisualIDRegistry.getVisualID(node)) {
			case ClassroomClassroomStudentsCompartmentEditPart.VISUAL_ID:
				for (Iterator cit = node.getChildren().iterator(); cit
						.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (SchoolVisualIDRegistry.getVisualID(cnode)) {
					case StudentEditPart.VISUAL_ID:
						for (Iterator it = cnode.getTargetEdges().iterator(); it
								.hasNext();) {
							Edge incomingLink = (Edge) it.next();
							if (SchoolVisualIDRegistry
									.getVisualID(incomingLink) == StudentFriendsEditPart.VISUAL_ID) {
						...
						for (Iterator it = cnode.getSourceEdges().iterator(); it
							...
							if (SchoolVisualIDRegistry
									.getVisualID(outgoingLink) == StudentFriendsEditPart.VISUAL_ID) {
								DestroyReferenceRequest r = new DestroyReferenceRequest(
